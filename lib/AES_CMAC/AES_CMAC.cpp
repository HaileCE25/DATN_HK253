/*
   Original: Copyright (c) 2020 Boot&Work Corp., S.L. All rights reserved
   (IndustrialShields/arduino-AES_CMAC, redistributed via Obsttube/AES_CMAC)
   ...
   ---
   Modified for project UWB_TX: constructor/member generalized from
   AESTiny128& to BlockCipher& and setKey() length taken from
   cipher.keySize() so this also works with AESTiny256 (AES-256-CMAC),
   as required by the CCC key-derivation (mURSK/dURSK) code. The CMAC
...
////////////////////////////////////////////////////////////////////////////////////////////////////
AES_CMAC::AES_CMAC(BlockCipher& cipher) : cipher(cipher) {
...
void AES_CMAC::generateMAC(uint8_t* mac, const uint8_t* key, const uint8_t* data, size_t dataLen) {
	...
	cipher.encryptBlock(mac, const_Zero);
	for (int i = 0; i < 16; ++i) {
		X[i] = mac[i];
	}
