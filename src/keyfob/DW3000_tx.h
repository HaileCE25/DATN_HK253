...
public:
	...
	static void setTXFrame(unsigned long long frame_data,uint16_t datalenght ,uint8_t const * data_buffer);
  ...
  static unsigned long long readSysTimestamp();
  ...
  static void write_data(int base, int sub,const uint8_t* data, int data_len);
  ...
  static void PrepareFinalData(uint8_t* Data);
  static void PreparePrepollData(uint8_t* Data);
	...
	static void WriteRespondSTS_IV(uint8_t number_resp);
  ...
  static void WriteFinalSTS_IV();
	...
	static void SendRespond(uint8_t number_resp);
...
private:
	...
	static uint32_t readOrWriteFullAddress(uint32_t base, uint32_t sub, uint32_t data, uint32_t data_len, uint32_t readWriteBit, uint8_t *buffer,const uint8_t* data_buffer);
	...
	static uint32_t sendBytes(uint8_t b[], int lenB, int recLen, uint8_t *Data);
...
typedef struct
{
	...
	uint32_t RespondTimestamp_H_u32[8];
	uint32_t RespondTimestamp_L_u32[8];
